...
* `<ModelName>CrossAttention`
    * The QKV computation here is currently inefficient, for reasons described [later](#parallelizing-cross-attention----parameter-matrices). Addressing this is a near-term workstream.
    * Members
        * `qkv_proj`: $[W_Q W_K W_V]$ as `QKVParallelLinear` instance
        * `attn`: `Attention` instance
        * `out_proj`: $W_O$ as `RowParalleLinear` instance

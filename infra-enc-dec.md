...
<figure>
  ...
  <figcaption style="text-align: center; margin-top: 10px;">
    <strong>Figure 2:</strong> Block-diagonal attention masks employed in the vLLM backend. *Left*: non-causal encoder attention for $N=4$ concatenated sequences each with length $e_i$ for $i \in [0,N)$. *Middle*: causal decoder self-attention for $N=4$ concatenated sequences each with length $d_i$ for $i \in [0,N)$. *Right*: non-causal encoder/decoder cross-attention between $N=4$ decoder sequences each with length $d_i$ for $i \in [0,N)$ and $N=4$ encoder sequences each with length $e_i$ for $i \in [0,N)$.
  </figcaption>
</figure>
